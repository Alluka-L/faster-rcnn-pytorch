...
def bbox_transform_inv(bboxes, deltas, batch_size):
...
def bbox_overlaps(anchors, gt_boxes):
    ...
    :param gt_boxes: (K, 4) ndarray of float
...
def bbox_overlaps_batch(anchors, gt_boxes):
    ...
    :return overlaps: (N, K) ndarray of overlap between boxes and query _boxes
    """
    ...
    if anchors.dim() == 2:
        ...
        anchors_boxes_area = (anchors_boxes_x * anchors_boxes_y).view(batch_size, N, 1)

        gt_area_zero = (gt_boxes_x == 1) & (gt_boxes_y == 1)
        anchors_area_zero = (anchors_boxes_x == 1) & (anchors_boxes_y == 1)

        boxes = anchors.view(batch_size, N, 1, 4).expand(batch_size, N, K, 4)
        query_boxes = gt_boxes.view(batch_size, 1, K, 4).enpand(batch_size, N, K, 4)
        ...
        ua = anchors_boxes_area + gt_boxes_area - (iw * ih)
        overlaps = iw * ih / ua

        # mask the overlap here.
        overlaps.masked_fill(gt_area_zero.view(batch_size, 1, K).expand(batch_size, N, K), 0)
        overlaps.masked_fill(anchors_area_zero.view(batch_size, N, 1).expand(batch_size, N, K), -1)
